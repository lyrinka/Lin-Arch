// lyrinka kernal CtxSw Framework version 1.1.0 .C file +nc from lyrinka 1.0.6 .C file +nc 
//#ifndef __ctxsw_H__
//#define __ctxsw_H__
#include <stm32f10x.h> 
#include <CtxSw config.h> 																	// Macro Required: SVCn_CtxSwTrig, SVCn_CtxSwSwitch 
void SVC_ProxyCaller(u8 ID, u32 * StkF); 										// External Functions - SVC Proxy Caller 
void CtxSw_Init(void); 																			// User Functions - NVIC and Handler Initialization 
void CtxSw_ThreadInit(u32 **, void *, void *); 							// User Functions - New Thread Stack Init with Loop Function 
void __svc(SVCn_CtxSwTrig) 		CtxSw_Enter(u32 **, u32 **); 	// User Functions - Enter First Thread from Main 
void __svc(SVCn_CtxSwSwitch) 	CtxSw_Switch(u32 **); 				// User Functions - Switch to any Thread from Thread 
void CtxSw_SwitchISR(u32 **); 															// ISR  Functions - Switch to any Thread from ISR 
void __svc(SVCn_CtxSwTrig) 		CtxSw_Exit(u32 **); 					// User Functions - Exit from Thread to Main 
//////////// 
u32 * ptrC, * ptrN; 		// Internal Variables - The pointer to the current and the next thread stack pointer 
__asm void CtxSw_ThreadInit(u32 ** funcStk, void * funcPtr, void * loopPtr){ 
		// Initialize the stack for a new thread by the pointer to the stack pointer. 
		...
		// void Thread(u32 Arg0, u32 Arg1, u32 Counter, void * LoopPoint){...} 
		LDR		R3, [R0] 
		...
		MOV		R1, #0 
		STR		R1, [R3, #44] 		// Clear Counter Arg 
		STR		R1, [R3, #72] 		// Clear Counter Variable 
		BX		LR 
		NOP 
ThreadExit 
		LDR		LR, =ThreadExit 
...
__asm void CtxSw_ThreadSetArgs(u32 ** funcStk, u32 Arg0, u32 Arg1){ 
		// Load the two arguments into the the stack of a new thread. 
		// The stack pointer must be initialized first. 
		LDR		R3, [R0] 
...
Cut 															// 1. Match Switch or FinState of OsEnter 
		...
		EXPORT		CtxSw_SwitchISR 
CtxSw_SwitchISR 							// ISR Functions - Switch to any Thread from ISR 
		LDR		R2, =ptrN 
		LDR		R1, [R2] 						// Get ptrN 
